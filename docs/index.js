...
"url":0,
"doc":"A shortcut for  item(description).apply(f) and spares  f the trouble of outputting a  Result object. >>> p1 = argument(\"foo\") >>> p1.parse_args(\"bar\", return_dict=False) [('foo', 'bar')] Here we use  f to directly manipulate the binding generated by  item : >>> p2 = apply(lambda bar: [KeyValue(bar + \"e\", bar + \"f\")], description=\"baz\") >>> p2.parse_args(\"bar\", return_dict=False) [('bare', 'barf')]",
"func":1
},
{
...
"url":0,
"doc":" done succeds on the end of input and fails on everything else. >>> done().parse_args() {} >>> done().parse_args(\"arg\") Unrecognized argument: arg Without  done the parser will not complain about leftover (unparsed) input: >>> flag(\"verbose\").parse_args(\" verbose\", \" quiet\", allow_unparsed=True) {'verbose': True} When  allow_unparsed=False (the default),  parse_args adds  >> done() to the end of the parser: >>> flag(\"verbose\").parse_args(\" verbose\", \" quiet\", allow_unparsed=False) usage:  verbose Unrecognized argument:  quiet >>> (flag(\"verbose\") >> done( .parse_args(\" verbose\", \" quiet\", allow_unparsed=True) usage:  verbose Unrecognized argument:  quiet   quiet is not parsed here but this does not cause the parser to fail. If we want to prevent leftover inputs, we can use  done : >>> (flag(\"verbose\") >> done( .parse_args(\" verbose\", \" quiet\") usage:  verbose Unrecognized argument:  quiet  done is usually necessary to get  nonpositional to behave in the way that you expect. See  nonpositional API docs for details.",
"func":1
},
{
