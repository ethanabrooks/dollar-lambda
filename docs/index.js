...
"url":0,
"doc":"Tries apply the first parser. If it fails, tries the second. If that fails, the parser fails. >>> from dollar_lambda import argument, option, done, flag >>> p = option(\"option\") | flag(\"verbose\") >>> p.parse_args(\" option\", \"x\") {'option': 'x'} >>> p.parse_args(\" verbose\") {'verbose': True} Note that by default,  parse_args adds  >> done to the end of parsers, causing  p to fail when both arguments are supplied: >>> p.parse_args(\" verbose\", \" option\", \"x\") usage: [ option OPTION |  verbose] Unrecognized argument:  option To disable this behavior, use  allow_unparsed : >>> p.parse_args(\" verbose\", \" option\", \"x\", allow_unparsed=True) {'verbose': True}",
"func":1
},
{
...
"url":0,
"doc":"Applies  parser , applies a predicate to the result and fails if this returns false. >>> p = option(\"x\", type=int).many().sat(  . lambda kvs: sum([kv.value for kv in kvs]) > 0,  . lambda x: ArgumentError(f\"The values in {list(x)} must sum to more than 0.\"),  . ) >>> p.parse_args(\"-x\", \"-1\", \"-x\", \"1\")  fails usage: [-x X  .] The values in [KeyValue(key='x', value=-1), KeyValue(key='x', value=1)] must sum to more than 0. >>> p.parse_args(\"-x\", \"-1\", \"-x\", \"2\")  succeeds {'x': 2} Parameters      parser : Parser[A] The parser to apply. predicate : Callable A], bool] The predicate to apply to the result of  parser .  sat fails if this predicate returns false. on_fail : Callable A], ArgumentError] A function producing an ArgumentError to return if the predicate fails. Takes the output of  parser as an argument.",
"func":1
},
{
"ref":"dollar_lambda.Parser.type",
"url":0,
"doc":"A wrapper around  apply that simply applies  f to the value of the most recently parsed input. >>> p1 = option(\"x\") >> option(\"y\") >>> p = p1.type(int) >>> p.parse_args(\"-x\", \"1\", \"-y\", \"2\")  converts \"1\" but not \"2\" {'y': '2', 'x': 1}",
"func":1
},
{
...
"url":0,
"doc":"A shortcut for   item(description).apply(f)   and spares  f the trouble of outputting a  Result object. Here is an example of usage. First we define a simple  argument parser: >>> p1 = argument(\"foo\") >>> p1.parse_args(\"bar\", return_dict=False) [('foo', 'bar')] Here we use  f to directly manipulate the binding generated by  item : >>> p2 = apply(lambda bar: [KeyValue(bar + \"e\", bar + \"f\")], description=\"baz\") >>> p2.parse_args(\"bar\", return_dict=False) [('bare', 'barf')]",
"func":1
},
{
...
"url":0,
"doc":" done succeds on the end of input and fails on everything else. >>> done().parse_args() {} >>> done().parse_args(\"arg\") Unrecognized argument: arg Without  done the parser will not complain about leftover (unparsed) input: >>> flag(\"verbose\").parse_args(\" verbose\", \" quiet\", allow_unparsed=True) {'verbose': True} When  allow_unparsed=False (the default),  parse_args adds  >> done() to the end of the parser:",
"func":1
},
{
"ref":"dollar_lambda.matches",
"url":0,
"doc":"Checks if the next word is  s . >>> matches(\"hello\").parse_args(\"hello\") {'hello': 'hello'} >>> matches(\"hello\").parse_args(\"goodbye\") usage: hello Expected 'hello'. Got 'goodbye' Parameters      s: str The word to that input will be checked against for equality. peak : bool If  False , then the parser will consume the word and return the remaining words as  unparsed . If  True , then the parser leaves the  unparsed component unchanged. regex : bool Whether to treat  s as a regular expression. If  False , then the parser will only succeed on string equality. Examples     >>> p = matches(\"hello\") >> matches(\"goodbye\") >>> p.parse_args(\"hello\", \"goodbye\") {'hello': 'hello', 'goodbye': 'goodbye'} Look what happens when  peak=True : >>> p = matches(\"hello\", peak=True) >> matches(\"goodbye\") >>> p.parse_args(\"hello\", \"goodbye\") usage: hello goodbye Expected 'goodbye'. Got 'hello' The first parser didn't consume the word and so \"hello\" got passed on to  equals(\"goodbye\") . But this would work: >>> p = matches(\"hello\", peak=True) >> matches(\"hello\") >>matches(\"goodbye\") >>> p.parse_args(\"hello\", \"goodbye\") {'hello': 'hello', 'goodbye': 'goodbye'}",
"func":1
},
{
...
"url":0,
"doc":" nonpositional takes a sequence of parsers as arguments and attempts all permutations of them, returning the first permutations that is successful: >>> p = nonpositional(flag(\"verbose\"), flag(\"quiet\" >>> p.parse_args(\" verbose\", \" quiet\") {'verbose': True, 'quiet': True} >>> p.parse_args(\" quiet\", \" verbose\")  reverse order also works {'quiet': True, 'verbose': True} Parameters      repeated : Optional[Parser[Sequence[A ] If provided, this parser gets applied repeatedly (zero or more times) at all positions. Examples     >>> p = nonpositional(repeated=flag(\"x\" >>> p.parse_args(return_dict=False) [] >>> p.parse_args(\"-x\", return_dict=False) [('x', True)] >>> p.parse_args(\"-x\", \"-x\", return_dict=False) [('x', True), ('x', True)] >>> p = nonpositional(flag(\"y\"), repeated=flag(\"x\" >>> p.parse_args(\"-y\", return_dict=False) [('y', True)] >>> p.parse_args(\"-y\", \"-x\", return_dict=False) [('y', True), ('x', True)] >>> p.parse_args(\"-x\", \"-y\", return_dict=False) [('x', True), ('y', True)] >>> p.parse_args(\"-y\", \"-x\", \"-x\", return_dict=False) [('y', True), ('x', True), ('x', True)] >>> p.parse_args(\"-x\", \"-y\", \"-x\", return_dict=False) [('x', True), ('y', True), ('x', True)] >>> p.parse_args(\"-x\", \"-x\", \"-y\", return_dict=False) [('x', True), ('x', True), ('y', True)] >>> p = nonpositional(flag(\"y\"), repeated=(flag(\"x\") | flag(\"z\" .ignore( >>> p.parse_args(\"-x\", \"-y\", \"-z\", return_dict=False) [('y', True)]",
"func":1
},
{
...
"url":0,
"doc":"A decorator for adding a function as a child of this tree. As a subcommand, the function's name must be invoked on the command line for the function to be called. Parameters      can_run: bool Whether the parser will permit the decorated function to run if no further arguments are supplied. flip_bools: bool Whether to add   no-  before arguments that default to  True . help: Dict[str, str] A dictionary of help strings for the arguments. repeated: Optional[Parser[Sequence[KeyValue[Any  If provided, this parser gets applied repeatedly (zero or more times) at all positions. See  nonpositional for examples. parsers: Dict[str, Parser] A dictionary reserving arguments for custom parsers. See below for examples. See  command for examples. Examples     With  flip_bools set to  True : >>> tree = CommandTree()  . >>> @tree.subcommand(flip_bools=True)  . def f1(b: bool = True):  . return dict(f1=dict(b=b  . >>> tree(\"-h\") usage: f1  no-b b: (default: True) With  flip_bools set to  False : >>> tree = CommandTree()  . >>> @tree.subcommand(flip_bools=False)  . def f1(b: bool = True):  . return dict(f1=dict(b=b  . >>> tree(\"-h\") usage: f1 -b b: (default: True) With  can_run set to  True (the default), we can run  f1 by not passing arguments for the  f1 's children: >>> tree = CommandTree()  . >>> @tree.subcommand(can_run=True)   >> @f1.subcommand()  . def g1(n: int):  . return dict(g1=dict(b=b, n=n  . >>> tree(\"-h\") usage: f1 -b g1 -n N >>> tree(\"f1\", \"-b\") {'f1': {'b': True With  can_run set to  False , the parser will fail if the child function arguments are not supplied: >>> tree = CommandTree()  . >>> @tree.subcommand(can_run=False)   >> @f1.subcommand()  . def g1(n: int):  . return dict(g1=dict(b=b, n=n  . >>> tree(\"-h\") usage: f1 -b g1 -n N >>> tree(\"f1\", \"-b\") usage: f1 -b g1 -n N The following arguments are required: g1",
"func":1
},
{
...
"url":4,
"doc":"SuccessError(usage: str, input: 'Sequence[str]', output: 'NonemptyList[Parse[A ')"
},
{
