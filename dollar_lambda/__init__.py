...
>>> @command()
... def main(x: int, y: int, dev: bool = False, prod: bool = False):
...     return dict(x=x, y=y, dev=dev, prod=prod)
...
let's say we wanted to execute a `prod` function when the user provides the
`--prod` flag, and a `dev` function when the user provides the `--dev` flag:

>>> tree = CommandTree()
...
>>> @tree.command()
... def base_function(x: int, y: int):  # common arguments that must precede child arguments
...     raise RuntimeError("Does not execute.")
...
>>> @base_function.command()
... def prod(x: int, y: int, prod: bool):
...     print("Ran prod function with arguments:", dict(x=x, y=y, prod=prod))
...
>>> @base_function.command()
... def dev(x: int, y: int, dev: bool):
...     print("Ran dev function with arguments:", dict(x=x, y=y, dev=dev))
...
>>> tree("-x", "1", "-y", "2", "--prod")
Ran prod function with arguments: {'x': 1, 'y': 2, 'prod': True}

If we provide the `--quiet` flag, `$λ` invokes the `quiet_function`:

>>> tree("-x", "1", "-y", "2", "--dev")  # execute quiet_function
Ran dev function with arguments: {'x': 1, 'y': 2, 'dev': True}

With this configuration, the parser will fail if neither
`--verbose` or `--quiet` is given:

>>> tree("-x", "1", "-y", "2")  # fails
usage: -x X -y Y [--prod | --dev]
The following arguments are required: --prod
