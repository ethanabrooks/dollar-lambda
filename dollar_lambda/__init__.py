...
`$λ` was built with minimal dependencies from functional first principles. As a
result, it is the most
...
`$λ` provides high-level functionality equivalent to other parsers. But unlike
other parsers, it permits low-level customization to handle arbitrarily complex
parsing patterns. As we’ll see in the tutorial, there are many parsing patterns
that `$λ` can handle which are not possible with other parsing libraries.
...
As we’ll demonstrate in the tutorial, `$λ` provides three main syntactic
shortcuts for cutting down boilerplate:

- operators like `>>`, `|`, and `+` (and `>=` if you want to get fancy)
- the `command` decorator and the `CommandTree` object for building tree-shaped
  parsers
- the `Args` syntax built on top of python `dataclasses`.

As a rule, `$λ` avoids reproducing python functionality and focuses on the main
job of an argument parser: parsing. Arguably, `$λ` is way more expressive than
any reasonable person would ever need… but even if it’s not the parser that we
need, it’s the parser we deserve.
...
not bind any values to variables, but causes the parser to fail in some cases.
...
`option` is similar but takes an argument: By default, `option` expects a single
`-` for single-character variable names (as in `-x`), as opposed to `--` for
longer names (as in `--xenophon`):
...
`--quiet` is not parsed here but this does not cause the parser to fail.  If we
want to prevent leftover inputs, we can use `done`:
...
The `|` operator is used for alternatives. Specifically, it will try the first
parser, and if that fails, try the second:
...
The `>>` operator is used for sequential composition. It applies the first
parser and then hands the output of the first parser to the second parser. If
either parser fails, the composition fails:
...
`nonpositional` takes a sequence of parsers as arguments and attempts all
permutations of them, returning the first permutations that is successful:
...
For just two parsers you can use `+`, or `Parser.__add__`, instead of
`nonpositional`:
...
If alternatives or defaults appear among the arguments to `nonpositional`, you
will probably want to add `>>` followed by `done` (or another parser) after
`nonpositional`. Otherwise, the parser will not behave as expected:
...
In our example, both permutations are actually succeeding. This first succeeds
by falling back to the default, and leaving the last word of the input,
`--verbose`, unparsed.  Either interpretation is valid, and `nonpositional`
returns one arbitrarily—just not the one we expected.

Now let’s add `>> done()` to the end:

>>> p = nonpositional(flag("verbose", default=False), flag("quiet")) >> done()

This ensures that the first permutation will fail because the leftover
`--verbose` input will cause the `done` parser to fail:
...
As we’ve seen `flag("verbose") | flag("quiet")` succeeds on either `--verbose`
or `--quiet` (but one or the other is required).

`option("x", type=int)` succeeds on `-x X`, where `X` is some integer, binding
that integer to the variable `"x"`. Similarly for `option("y", type=int)`.
...
and applies them in every order, until some order succeeds.  Finally `done()`
ensures that only one of these parser permutations will succeed, preventing
ambiguity.

## Alternative syntax

There are a few alternative ways to express the functionality that we’ve seen so
far. We will start with the most succinct, but least flexible implementation and
then demonstrate some alternatives that tradeoff succinctness for flexibility.

### `command`

`command` is a decorator that creates a parser automatically based on the
...
Note that unlike our previous implementation, this will not complain if both or
neither `--verbose` and `--quiet` are given:
...
Note that `verbose_function` must include all the arguments from
`base_function`. And we make `quiet_function` a second branch:

>>> @base_function.command()
... def quiet_function(x: int, y: int, quiet: bool):
...     print(dict(x=x, y=y, quiet=quiet))

Again we include all arguments from `base_function`. Now we can invoke
`tree.main` to dynamically dispatch the parsed arguments to the function whose
corresponding parser succeeds first:
...
To make one or the other flag required, the `command` method takes a `required`
argument.
...
Note that all children must be required or else `base_function` will execute in
the absence of any flags.

`CommandTree` is especially useful when you want different parse-results to
invoke different functions.  One drawback of `CommandTree` is that it cannot be
freely combined with other parsers.  Parsers produced by `CommandTree` are
specialized for use with decorated functions and don’t play well with more
general-purpose parsers.

### `Args`

`Args` is a lighter-weight alternative, providing mostly syntactic sugar and a
bit of logic around the `nonpositional` function:
...
Make sure to import `field` from `dollar_lambda`, not from `dataclasses`.  Now
we can use `MyArgs.parser()` in combinations with other parsers:
...
What if there was a special argument, `verbosity`, that only makes sense if the
user chooses `--verbose`?
...
Remember that `+` or `Parser.__add__` evaluates two parsers in both orders and
stopping at the first order that succeeds. So this allows us to supply
`--verbose` and `--verbosity` in any order.
...
What if we want to specify verbosity by the number of times that `--verbose`
appears? For this we need `Parser.many`. Before showing how we could use
`Parser.many` in this setting, let’s look at how it works.
...
As you can see, `return_dict=False` returns a list of tuples instead of a dict,
so that you can have duplicate keys.
...
it fails when on zero successes (recall that `Parser.many` always succeeds). So
if `Parser.many` is like regex `*`, `Parser.many1` is like `+`. Take a look:
